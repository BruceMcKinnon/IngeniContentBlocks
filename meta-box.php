...
function ingeni_cpw_shortcode_meta_box( $post ) { ?>
	<p><?php esc_html_e( 'You can place this content block into your posts, pages, custom post types or widgets using the shortcode below:','ingeni-content-blocks-widget' ); ?></p>
	<code class="cpw-code" id="cpw-shortcode-1"><?php echo '[content_block id=' . esc_attr($post -> ID) . ']'; ?></code>
	<span class="cpw-clipboard" data-clipboard-target="#cpw-shortcode-1"><?php esc_html_e( 'Copy to clipboard', 'ingeni-content-blocks-widget' ); ?></span>

	<p><?php esc_html_e( 'Shortcode to use if you prefer using the slug instead of the post ID:','ingeni-content-blocks-widget' ); ?></p>
	<code class="cpw-code" id="cpw-shortcode-2"><?php echo '[content_block slug=' . esc_html ($post -> post_name) . ']'; ?></code>
	<span class="cpw-clipboard" data-clipboard-target="#cpw-shortcode-2"><?php esc_html_e( 'Copy to clipboard', 'ingeni-content-blocks-widget' ); ?></span>

	<p><?php esc_html_e( 'Use this shortcode to include the content block title:','ingeni-content-blocks-widget' ); ?></p>
	<code class="cpw-code" id="cpw-shortcode-3"><?php echo '[content_block id=' . esc_html ($post -> ID) . ' title=yes title_tag=h3]'; ?></code>
	<span class="cpw-clipboard" data-clipboard-target="#cpw-shortcode-3"><?php esc_html_e( 'Copy to clipboard', 'ingeni-content-blocks-widget' ); ?></span>
<?php
}
...
function ingeni_cpw_info_meta_box( $post ) {
	...
	echo '<p>' . esc_html__( 'You can use this field to describe this content block:','ingeni-content-blocks-widget' ) . '</p>';
...
function ingeni_cpw_save_postdata( $post_id ) {
	...
	$nonce = sanitize_text_field( wp_unslash($_POST['ingeni_cpw_info_meta_box_nonce']));
	...
	if ( isset( $_POST['post_type'] ) && 'ingeni_content_block' == $_POST['post_type'] ) {
	...
	switch ( $column_name ) {
		case 'ingeni_content_block_information' :
			if ( !empty( $custom_fields['_ingeni_content_block_information'][0] ) ) {
				echo esc_html($custom_fields['_ingeni_content_block_information'][0]);
			}
		break;
	}
}
