...
function add_content_block_icon() {
	...
	<a id="add-content-block" class="button thickbox cpw-button" title="' . esc_html ("Add Content Block", 'ingeni-content-blocks-widget' ) . '" href="' . esc_url (plugins_url()) . 'popup.php?type=add_content_block_popup&amp;TB_inline=true&amp;inlineId=content-block-form">
		<div class="dashicons dashicons-screenoptions"></div>' . esc_html ("Add Content Block", "ingeni-content-blocks-widget") . '
	</a>';
}
...
function add_content_block_popup() { ?>
	<script>
		...
		function insertContentBlockShortcode() {
			if (typeof content_id === 'undefined') {
				alert( "<?php esc_html_e( 'Please select a Content Block', 'ingeni-content-blocks-widget' ); ?>" );
				return false;
			}
	...
	<div id="content-block-form" style="display: none;">
		<h3>
			<?php esc_html_e( 'Insert Content Block', 'ingeni-content-blocks-widget' ); ?>
		</h3>
		<p>
			<?php esc_html_e( 'Select a Content Block below to add it to your post or page.', 'ingeni-content-blocks-widget' ); ?>
		</p>
		<p>
			<select class="add-content-block-id" id="add-content-block-id" onchange="selectContentBlockId(this)">
				<option value="">
					<?php esc_html_e( 'Select a Content Block', 'ingeni-content-blocks-widget' ); ?>
				</option>
				<?php
					...
					if ( $content_block ) {
						...
						echo '<option value="">' . esc_html ( 'No content blocks available', 'ingeni-content-blocks-widget' ) . '</option>';
					};
				?>
			</select>
		</p>
		<p>
			<input type="button" class="button-primary" value="<?php esc_attr_e( 'Insert Content Block', 'ingeni-content-blocks-widget' ) ?>" onclick="insertContentBlockShortcode();"/>
		</p>
	</div>
